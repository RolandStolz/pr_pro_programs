...
def add_w5_sessions(program: Program, phase_1_program: Program) -> list[WorkoutSession]:
    ...
    d1 = (
        ...
        .add_component(
            exercise_group_from_prev_session(
                w4d1, [reverse_hyperextension, russian_twist], reps=(None, +2)
            )
        )
    )
    ...
    d4 = (
        ...
        .add_component(
            SingleExercise(exercise=deadlift).add_repeating_set(
                5, deadlift.create_set(5, percentage=0.8)
            )
        )
        # .add_component(
        #     exercise_group_from_prev_session(
        ...
        .add_component(
            SingleExercise(exercise=barbell_lunges).add_repeating_set(
                4, barbell_lunges.create_set(6, weight=60)
            )
        )
        # .add_component(
